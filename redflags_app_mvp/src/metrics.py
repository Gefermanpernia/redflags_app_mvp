...
def prepare_production_data(
    ...
    prod["agent_key"] = prod.apply(
        lambda row: build_agent_key(
            row["agent_name"], row["hierarchy"], alias_mapping=alias_mapping
        ),
        axis=1,
    )
    ...
    prod["prev_mtd"] = prod.groupby(["month", "agent_key"])["production_mtd"].shift(
        fill_value=0
    )
    prod["production_weekly_closed"] = (prod["production_mtd"] - prod["prev_mtd"]).clip(
        lower=0
    )
    ...
    prod["production_monthly_total"] = prod.groupby(["month", "agent_key"])[
        "production_mtd"
    ].transform("max")

    prod["agent_name"] = prod["agent_name"].apply(
        lambda value: resolve_alias(value, alias_mapping)
    )
...
def prepare_appointments_data(
    ...
    appt["appointments_month_total"] = appt.groupby(["month", "agent_key"])[
        "appointments"
    ].transform("sum")
    appt["agent_name"] = appt["agent_name"].apply(
        lambda value: resolve_alias(value, alias_mapping)
    )
...
def build_weekly_dataset(
    ...
    weekly["agent_name"] = (
        weekly["agent_name_prod"].combine_first(weekly["agent_name_appt"]).fillna("")
    )
    weekly["hierarchy"] = (
        weekly["hierarchy_prod"].combine_first(weekly["hierarchy_appt"]).fillna("")
    )
    ...
    weekly["appointments_month_total"] = weekly.groupby(["month", "agent_key"])[
        "appointments"
    ].transform("sum")
    weekly["production_monthly_total"] = weekly.groupby(["month", "agent_key"])[
        "production_mtd"
    ].transform("max")
    return weekly
...
def build_summary_table(
    ...
    base = (
        weekly_df[["month", "agent_key", "agent_name", "hierarchy"]]
        .drop_duplicates()
        .copy()
    )
    ...
    flags_summary = flags_df.groupby(["month", "agent_key"], as_index=False).agg(
        active_flags=(
            "flag_name",
            lambda values: " | ".join(sorted(set(map(str, values)))),
        ),
    ...
    summary["risk_score"] = summary["risk_score"].fillna(0).astype(int)
    return summary.sort_values(["month", "agent_name"]).reset_index(drop=True)
