...
RULE_POINTS = {"RF-001": 25, "RF-002": 30, "RF-003": 20, "RF-OBS": 8}
...
def evaluate_red_flags(weekly_df: pd.DataFrame, monthly_df: pd.DataFrame, config: ThresholdConfig) -> pd.DataFrame:
    ...
    for _, row in monthly_df.iterrows():
        ...
        completed = group[group.get("is_completed_week", True)] if "is_completed_week" in group.columns else group
            ...
            if last_week_production >= config.spike_last_week_threshold and prev_production_total <= config.insignificant_production_threshold:
                ...
                flags.append(
                    _flag_record(
                        ...
                        reason="Pico semanal sin actividad previa.",
                        metrics=metrics,
                    )
                )

    for _, row in weekly_df.iterrows():
        if not bool(row.get("is_completed_week", True)):
            continue
        ...
        if weekly_appointments <= config.few_appointments_threshold and weekly_production >= config.weekly_production_suspicious:
            metrics = {
                ...
                "strict_weekly_threshold": config.weekly_production_strict,
            }
            flags.append(
                _flag_record(
                    month=row["month"],
        ...
        elif weekly_appointments <= config.few_appointments_threshold and config.weekly_observation_floor <= weekly_production < config.weekly_production_suspicious:
            metrics = {
                "week": int(row["week"]),
                "weekly_appointments": weekly_appointments,
                "weekly_production": weekly_production,
                "weekly_threshold": config.weekly_observation_floor,
            }
            flags.append(
                _flag_record(
                    month=row["month"],
                    ...
                    hierarchy=row["hierarchy"],
                    flag_id="RF-OBS",
                    flag_name="Caso de observación",
                    scope="semanal",
                    severity=config.severity_observation,
                    reason="Producción intermedia con citas bajas.",
                    metrics=metrics,
                )
            )
