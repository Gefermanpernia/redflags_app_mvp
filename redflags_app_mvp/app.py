...
from src.normalization import build_agent_key
...
def build_threshold_config() -> ThresholdConfig:
    with st.sidebar:
        ...
        appointments_merge_rule = st.selectbox(
            ...
            format_func=lambda value: "Manual sobreescribe Excel"
            if value == "overwrite"
            else "Manual + Excel (sumar)",
            index=0,
        )
...
def render_manual_load() -> None:
    ...
    labels = (
        [
            f"{row.agent_name} ({row.agent_code})"
            if str(row.agent_code).strip()
            else row.agent_name
    ...
    selected_label = st.selectbox(
        "Agente (catálogo)",
        options=[""] + labels,
        help="Selecciona un agente existente o completa nombre/código manualmente.",
    )
    ...
    agent_name = st.text_input(
        "Nombre de agente",
        value=selected_row["agent_name"] if selected_row is not None else "",
        key="manual_agent_name",
    )
    agent_code = st.text_input(
        "Código de agente (opcional)",
        value=selected_row["agent_code"] if selected_row is not None else "",
        key="manual_agent_code",
    )
    appointment_date = st.date_input(
        "Fecha del día",
        value=datetime.now().date(),
        key="manual_appointment_date",
    )
    appointment_count = st.number_input(
        "Cantidad de citas",
        min_value=0.0,
        step=1.0,
        key="manual_appointment_count",
    )
...
def main() -> None:
    ...
    tabs = st.tabs(["Carga", "Carga manual", "Dashboard", "Detalle", "Reportes", "Histórico"])
    with tabs[0]:
